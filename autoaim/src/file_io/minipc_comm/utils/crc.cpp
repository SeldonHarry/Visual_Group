...
// len=size(data)+size(check)
uint8_t CRC8_Calc(uint8_t* data, uint32_t len) {
  uint8_t CRC8 = CRC8_base;
  uint8_t index;
  while (len--) {
    index = CRC8 ^ (*data++);
...
uint8_t CRC8_Verify(uint8_t* data, uint32_t len) {
  ...
  return (CRC8_Calc(data, len - 1) == data_CRC8);
}
...
uint8_t CRC16_Verify(uint8_t* data, uint32_t len) {
  ...
  uint16_t data_CRC16;
  memcpy(&data_CRC16, &data[len - 2], 2);
  return (CRC16_Calc(data, len - 2) == data_CRC16);
}
...
void CRC16_Append(uint8_t* data, uint32_t len) {
  ...
  uint16_t CRC16 = CRC16_Calc(data, len - 2);
  memcpy(&data[len - 2], &CRC16, 2); // 占用数据结尾的两位，所以生成包的时候不能够在crc后面再加消息了。
}