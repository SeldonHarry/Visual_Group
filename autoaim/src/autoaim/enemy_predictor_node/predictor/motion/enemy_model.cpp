...
Sentry::Sentry(aimer::CoordConverter* const converter,
    ...
    converter(converter),
    state(state),
    armor_model(converter,
                state,
                aimer::MODEL_ARMOR_CREDIT_TIME.at(aimer::ModelType::SENTRY),
                std::move(param_visitor),
                debugger),
...
Infantry::Infantry(aimer::CoordConverter* const converter,
    ...
    armor_model(converter,
                state,
                aimer::MODEL_ARMOR_CREDIT_TIME.at(aimer::ModelType::INFANTRY),
                std::move(param_visitor),
                debugger),
...
void Infantry::update() {
    ...
    this->cj_top_model.update(this->converter, this->state);
}

// send 的 shoot 也需要我处理
aimer::AimInfo Infantry::get_aim() const {
    if (this->cj_top_model.get_top_level() >= 1) {
        return this->cj_top_model.get_aim(this->converter, this->state);
    }
...
void Infantry::draw_aim(cv::Mat& img) const {
    ...
    // }
    this->cj_top_model.draw_armors(img, this->converter->get_img_t(), this->converter, this->state);
}
...
Outpost::Outpost(aimer::CoordConverter* const converter,
                 ...
                 const param::ParamVisitor& param_visitor,
                 std::shared_ptr<ros2addon::debug::Debugger> debugger):
    cj_top_model(std::move(param_visitor)),
    converter(converter),
    state(state),
    armor_model(converter,
                state,
                aimer::MODEL_ARMOR_CREDIT_TIME.at(aimer::ModelType::OUTPOST),
                std::move(param_visitor),
                debugger),
    top3_model(converter,
               state,
               /*armor_cnt=*/3,
               /*jump_angle=*/M_PI / 8.,
               /*min_active_rotate=*/1,
...
Statue::Statue(aimer::CoordConverter* const converter,
    ...
    armor_model(converter,
                state,
                aimer::MODEL_ARMOR_CREDIT_TIME.at(aimer::ModelType::STATUE),
                std::move(param_visitor),
                debugger) {}
...
const std::unordered_map<aimer::ModelType,
    ...
    MODEL_MAP = {
        { aimer::ModelType::SENTRY,
          [](aimer::CoordConverter* const converter,
             ...
             const param::ParamVisitor& param_visitor,
             std::shared_ptr<ros2addon::debug::Debugger> debugger) {
              return std::make_unique<Sentry>(converter, state, param_visitor, debugger);
          } },
        { aimer::ModelType::INFANTRY,
          [](aimer::CoordConverter* const converter,
             ...
             const param::ParamVisitor& param_visitor,
             std::shared_ptr<ros2addon::debug::Debugger> debugger) {
              return std::make_unique<Infantry>(converter, state, param_visitor, debugger);
          } },
        { aimer::ModelType::BALANCE_INFANTRY,
          [](aimer::CoordConverter* const converter,
             ...
             const param::ParamVisitor& param_visitor,
             std::shared_ptr<ros2addon::debug::Debugger> debugger) {
              return std::make_unique<BalanceInfantry>(converter, state, param_visitor, debugger);
          } },
        { aimer::ModelType::OUTPOST,
          [](aimer::CoordConverter* const converter,
             ...
             const param::ParamVisitor& param_visitor,
             std::shared_ptr<ros2addon::debug::Debugger> debugger) {
              return std::make_unique<Outpost>(converter, state, param_visitor, debugger);
          } },
        { aimer::ModelType::STATUE,
          [](aimer::CoordConverter* const converter,
             ...
             const param::ParamVisitor& param_visitor,
             std::shared_ptr<ros2addon::debug::Debugger> debugger) {
              return std::make_unique<Statue>(converter, state, param_visitor, debugger);
          } }
    };
...
EnemyModelFactory::create_model(aimer::CoordConverter* const converter,
                                aimer::EnemyState* const state,
                                const param::ParamVisitor& param_visitor,
                                std::shared_ptr<ros2addon::debug::Debugger> debugger) {
    return aimer::MODEL_MAP.at(aimer::ENEMY_TO_MODEL.at(
        state->get_enemy_type()))(converter, state, param_visitor, debugger);
}
} // namespace aimer
