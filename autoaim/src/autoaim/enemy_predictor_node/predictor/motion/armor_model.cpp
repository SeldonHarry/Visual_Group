...
void FilterThread::update(const aimer::ArmorData& armor, const double& t) {
    ...
    // 已经改成 ekf
    std::vector<double> q_vec = [this]() {
        double q_x = param::get_param<double>("auto-aim.armor-model.ekf.q.x", this->param_visitor_);
        double q_v = param::get_param<double>("auto-aim.armor-model.ekf.q.v", this->param_visitor_);
        return std::vector<double> { q_x, q_v, q_x, q_v, q_x, q_v };
    }();
    std::vector<double> r_vec {
        param::get_param<double>("auto-aim.armor-model.ekf.r.yaw", param_visitor_),
        param::get_param<double>("auto-aim.armor-model.ekf.r.pitch", param_visitor_),
        param::get_param<double>("auto-aim.armor-model.ekf.r.distance-at-1m", param_visitor_)
            * ypd.dis * ypd.dis
    };
...
/** @class MotionModel */
ArmorModel::ArmorModel(aimer::CoordConverter* const converter,
...
void ArmorModel::draw_aim(cv::Mat& img) const {
    for (const auto& d: this->filters) {
        ...
        cv::Scalar color = this->state->is_hit() ? ros2addon::debug::GRAY_MAIN_COLOR
            : this->converter->get_robot_status_ref().enemy_color == ::EnemyColor::BLUE
            ? ros2addon::debug::BLUE_MAIN_COLOR
            : ros2addon::debug::RED_MAIN_COLOR;
        ...
        debugger_->flask_map << debugger_->pos_pair_to_map_arrow(
            std::make_pair(pos, pos + v * 1.),
            color,
            ros2addon::debug::FLASK_MAP_THICKNESS
        );
        debugger_->flask_map << debugger_->pos_str_to_map_text(
            fmt::format("{}", this->state->get_number()),
            pos,
            color,
            ros2addon::debug::FLASK_MAP_TEXT_SCALE / 2.
        );
    }
}
...
void ArmorModel::update() {
    ...
    for (const auto& d: armors) {
        ...
        if (it == this->filters.end()) {
            this->filters.insert(std::make_pair(
                d.id,
                FilterThread(this->converter, d, this->credit_time, param_visitor_)));
...
aimer::AimInfo ArmorModel::get_positive_aim() const {
    ...
    for (const auto& d: this->filters) {
        if (!d.second.credit()
            || d.second.armor.info.area() < max_area
                    * param::get_param<double>("auto-aim.armor-model.target-area-ratio",
                                               param_visitor_))
        {
            continue;
        ...
        aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
            ...
            return aimer::AimInfo(
                ...
                ypd_v,
                shoot_param,
                this->converter->aim_error_exceeded(ypd,
        ...
        aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
            ...
            return aimer::AimInfo(
                ...
                this->converter->aim_error_exceeded(
                    aimer::math::YpdCoord(ypd.yaw,
                                          this->converter->get_control_aim_pitch0(),
                                          ypd.dis),
                    this->state->get_sample_armor_ref(),
                    param::get_param<double>("auto-aim.armor-model.passive-mode.aim.tracking-range",
                                             param_visitor_),
                    0.,
                    this->state->get_armor_pitch())
                    ? ::ShootMode::IDLE
                    ...
                    : this->converter->aim_error_exceeded(
                          ypd,
                          this->state->get_sample_armor_ref(),
                          param::get_param<double>(
                              "auto-aim.armor-model.passive-mode.aim.max-error",
                              param_visitor_),
                          0.,
                          this->state->get_armor_pitch())
                        ? ::ShootMode::TRACKING
