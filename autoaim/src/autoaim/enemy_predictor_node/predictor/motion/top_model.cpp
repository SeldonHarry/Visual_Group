...
aimer::AimInfo get_top_limit_aim(
    ...
    auto get_directs = [param_visitor](const std::vector<top::TopArmor>& results) -> std::vector<top::TopArmor> {
        std::vector<top::TopArmor> directs;
        for (const auto& res: results) {
            if (std::fabs(res.zn_to_v) < aimer::math::deg_to_rad(
                    param::get_param<double>("auto-aim.top-model.aim.max-orientation-angle",param_visitor)
                ))
            {
    ...
    // results 应该包含时间
    auto get_indirect_aim = [&converter, &state, &center_filter, &top_w, &get_direction,param_visitor](
                                const std::vector<top::TopArmor>& results
                            ) -> AimArmor {
        double zn_to_lim = get_direction(top_w) == top::TopDirection::CCW
            ? -param::get_param<double>("auto-aim.top-model.aim.max-orientation-angle",param_visitor) / 180. * M_PI
            : +param::get_param<double>("auto-aim.top-model.aim.max-orientation-angle",param_visitor) / 180.
                * M_PI;
        ...
        for (const auto& armor: results) {
            ...
            double leaving_angle =
                 ...
                 * param::get_param<double>("auto-aim.top-model.aim.max-out-error",param_visitor))
                / armor.length;
            double armor_to_lim =
        ...
        bool for_a_hit = // 1. 电机有没有跟上
            converter->aim_error_exceeded(
                ...
                || converter->aim_error_exceeded(
                    ...
                    state->get_sample_armor_ref(),
                    param::get_param<double>("auto-aim.top-model.aim.max-swing-error",param_visitor),
                    hit_aim.armor.zn_to_v,
                    state->get_armor_pitch()
                )
                ||
...
void top_draw_aim(
    ...
    debugger->flask_map << debugger->pos_str_to_map_text(
        fmt::format("{}", state->get_number()),
        center,
        ros2addon::debug::FLASK_MAP_TEXT_COLOR,
        ros2addon::debug::FLASK_MAP_TEXT_SCALE
    );
    ...
    for (int i = 0; i < size; ++i) {
        ...
        debugger->draw_lines(img, pts_vec[i], color_armor * coe_i, thickness, true);
        debugger->draw_line(
            img,
            pts_vec[i][3],
            pts_vec[p][0],
        ...
        debugger->flask_map << debugger->poses_to_map_lines(
            armors_corners[i],
            state->is_hit() ? ros2addon::debug::GRAY_MAIN_COLOR : color_armor,
            true,
            ros2addon::debug::FLASK_MAP_THICKNESS
        );
        debugger->flask_map << debugger->poses_to_map_lines(
            { armors_corners[i][3], armors_corners[p][0] },
            state->is_hit() ? ros2addon::debug::GRAY_MAIN_COLOR : color_sp,
            false,
            ros2addon::debug::FLASK_MAP_THICKNESS
        );
        debugger->flask_map << debugger->poses_to_map_lines(
...
bool SimpleRotateCounter::w_active() const { // 旋转状态
    return std::fabs(this->get_w())
        > param::get_param<double>("auto-aim.top-model.active-w",this->param_visitor_) / 180. * M_PI;
}

bool SimpleRotateCounter::w_inactive() const {
    return std::fabs(this->get_w())
        <= param::get_param<double>("auto-aim.top-model.inactive-w",this->param_visitor_) / 180. * M_PI;
}
...
SimpleTopModel::SimpleTopModel(
    ...
    counter(armor_cnt, jump_angle, min_active_rotate,param_visitor),
    ...
    // 计算和更新中心
    {
        ...
        if (this->state->get_enemy_type() == aimer::EnemyType::OUTPOST) {
            const double center_update_max_angle = aimer::math::deg_to_rad(
                param::get_param<double>("auto-aim.top-model.outpost.center-update-max-angle",param_visitor_)
            );
            ...
            this->center_filter.update(
                ...
                this->converter->get_img_t(),
                { 0.01, 10. },
                { param::get_param<double>("auto-aim.top-model.center-r-at-1m",param_visitor_) * center_pro.norm()
                  * center_pro.norm() }
            );
        }
    }
