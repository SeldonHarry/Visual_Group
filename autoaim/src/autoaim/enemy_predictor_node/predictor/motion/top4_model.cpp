...
AbManager::AbManager(const param::ParamVisitor& param_visitor): param_visitor_(param_visitor) {}
...
void AbManager::update(const Eigen::Vector3d& center, const top4::AbArmor& d) {
    auto get_param = [this](const std::string& name) -> double {
        return ::param::get_param<double>("auto-aim.top-model.ab." + name, this->param_visitor_);
    };
    ...
    this->z_filter.update(std::clamp(d.z_plus, -get_param("z.abs-max"), get_param("z.abs-max")),
                          0.,
                          { 1. },
...
RotateCounter::RotateCounter(const double& init_radius, const param::ParamVisitor& param_visitor):
ab_manager{AbManager(param_visitor),AbManager(param_visitor)}
{
    param_visitor_ = param_visitor;
...
bool RotateCounter::w_active() const { // 旋转状态
    return std::fabs(this->get_w())
        > param::get_param<double>("auto-aim.top-model.active-w",param_visitor_) / 180. * M_PI;
}

bool RotateCounter::w_inactive() const {
    return std::fabs(this->get_w())
        <= param::get_param<double>("auto-aim.top-model.inactive-w",param_visitor_) / 180. * M_PI;
}

bool RotateCounter::top_active() const {
    return !this->w_inactive()
        && std::fabs(this->get_active_rotate())
        >= param::get_param<int64_t>("auto-aim.top-model.top4.active-rotate",param_visitor_);
}
...
void RotateCounter::update_rotate(const std::vector<aimer::ArmorData>& armors) {
    ...
    } else if (armors.size() == 2 && this->pattern != top4::TopPattern::AB
...
void RotateCounter::update_ab(const Eigen::Vector3d& center,
...
TopModel::TopModel(aimer::CoordConverter* const converter,
    ...
    counter(state->get_default_radius(),param_visitor),
    param_visitor_(param_visitor),
    debugger_(debugger) {}
// 少了少了，常数又少啦
...
aimer::AimInfo TopModel::get_limit_aim() const {
    ...
    return top::get_top_limit_aim(this->center_filter,
                                  prediction_results,
                                  hit_results,
                                  this->counter.get_w(),
                                  this->converter,
...
void TopModel::update() { // 1 or 2
    ...
    debugger_->flask_aim << fmt::format("top_w {:.2f}", this->counter.get_w() / M_PI * 180.);
    if (this->counter.get_pattern() == top4::TopPattern::A) {
        debugger_->flask_aim << fmt::format("len in {:.3f}|#{:.3f}",
...
// 本函数禁用 this 以保证函数性
auto TopModel::length_sampling(const std::vector<aimer::ArmorData>& armors,
...
// 返回 z_to_l_fit 角
double TopModel::double_angle_sampling(const std::vector<aimer::ArmorData>& armors,
                                       const double& z_to_l_exp,
    ...
    double z_to_l_fit_by_r = [&]() {
        double z_to_r_fit = top::fit_single_z_to_v(armors[1],
                                                   z_to_l_exp + M_PI / 2,
                                                   M_PI,
                                                   M_PI / 2. * 3.,
                                                   converter);
    ...
    // 若直接加权，需保证符号为正
    double z_to_l_fit_weight = aimer::math::get_weighted_angle(z_to_l_fit_by_l,
                                                               armors[0].info.area(),
                                                               z_to_l_fit_by_r,
                                                               armors[1].info.area());
...
void TopModel::update_by_double_armors() {
    ...
    if (aimer::math::get_ratio(armors[0].info.area(), armors[1].info.area())
        > param::get_param<double>("auto-aim.top-model.radius-sampling-area-ratio",param_visitor_))
    {
        TopModel::LengthSample length_sample = this->length_sampling(
            armors,
            aimer::math::reduced_angle(this->counter.predict_theta_l(this->converter->get_img_t())
                                       - this->converter->get_camera_z_i_yaw()),
            this->converter); // 包括角度取样

        debugger_->flask_aim << fmt::format(
            "zn_to_l[D]: {:.2f}",
            aimer::math::reduced_angle(length_sample.z_to_l_fit + M_PI) / M_PI * 180.);
        debugger_->flask_aim << fmt::format("len sampLR {:.3f}|{:.3f}",
                                               length_sample.ab[0].length,
                                               length_sample.ab[1].length);

        this->counter.update_theta_l(length_sample.z_to_l_fit
                                         + this->converter->get_camera_z_i_yaw(),
                                     this->converter->get_img_t(),
                                     param::get_param<double>("auto-aim.top-model.single-angle-r",param_visitor_));
        this->counter.update_ab(length_sample.center,
                                length_sample.ab,
                                this->converter->get_img_t());
    } else {
        ...
        debugger_->flask_aim << fmt::format("zn_to_l[W]: {:.2f}",
                                               aimer::math::reduced_angle(z_to_l_fit + M_PI) / M_PI
                                                   * 180.);

        this->counter.update_theta_l(this->converter->get_camera_z_i_yaw() + z_to_l_fit,
                                     this->converter->get_img_t(),
                                     param::get_param<double>("auto-aim.top-model.double-angle-r",param_visitor_));
    }
    // 无论如何都要估计角度，更新中心
    double z_to_l_fix =
        aimer::math::reduced_angle(this->counter.predict_theta_l(this->converter->get_img_t())
    ...
    Eigen::Vector3d center_pro = (z_to_l_fix > M_PI / 4. * 3.)
        ? ([&]() { // 懒得传参数
              ...
              return top::prolonged_center(armors[0].info.pos,
        ...
        : ([&]() {
              ...
              return top::prolonged_center(armors[1].info.pos,
                                           this->counter.get_ab(r_index).length,
                                           r_norm2,
                                           this->counter.get_ab(r_index).z_plus);
          }());
    this->center_filter.update(center_pro,
                               this->converter->get_img_t(),
                               { 0.01, 10. },
                               { param::get_param<double>("auto-aim.top-model.center-r-at-1m",param_visitor_)
                                 * center_pro.norm() * center_pro.norm() });

    {
        // double 绘制反向延长中心
        auto cpt = this->converter->pi_to_pu(center_pro);
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(cpt, { 0, 255, 255 }, 6, 3);
    }
}
...
void TopModel::update_by_single_armor() {
    ...
    debugger_->flask_aim << fmt::format("zn_to_l[S]: {:.2f}",
                                           aimer::math::reduced_angle(z_to_l_fit + M_PI) / M_PI
                                               * 180.);

    this->counter.update_theta_l(z_to_l_fit + this->converter->get_camera_z_i_yaw(),
                                 this->converter->get_img_t(),
                                 param::get_param<double>("auto-aim.top-model.single-angle-r",param_visitor_));
    double z_to_l_fix =
        aimer::math::reduced_angle(this->counter.predict_theta_l(this->converter->get_img_t())
    ...
    Eigen::Vector3d center_pro = [&]() {
        ...
        Eigen::Vector2d l_norm2 =
            aimer::math::rotate(this->converter->get_camera_z_i2(), z_to_l_fix);
        return top::prolonged_center(armor.info.pos,
                                     this->counter.get_ab(l_index).length,
                                     l_norm2,
                                     this->counter.get_ab(l_index).z_plus);
    }();
...
void TopModel::draw_aim(cv::Mat& img) const {
    ...
    top::top_draw_aim(img, this->center_filter, res, this->active(), this->converter, this->state,this->debugger_);
    // 绘制中心
