#include "motion/cj_top_model.hpp"
...
Eigen::Vector3d ArmorFilter::predict_pos(const double& t) const {
    ...
    return cj::state_to_armor_pos(state);
}

Eigen::Vector3d ArmorFilter::predict_v(const double& t) const {
    const State state = this->predict(t);
    return cj::state_to_armor_v(state);
}
...
std::vector<State> state_vec_to_direct_state_vec(const std::vector<State>& state_vec,
...
AimAndState choose_direct_aim(const std::vector<State>& direct_state_vec,
...
// 只可以在角速度较大的时候调用哦
AimAndState choose_indirect_aim(const std::vector<State>& indirect_state_vec,
                                const double& max_orientation_angle,
    ...
    for (const auto& state: indirect_state_vec) {
        ...
        const double armor_to_wait = aimer::math::reduced_angle((w > 0.0 ? zn_to_wait - zn_to_armor
                                                                         : zn_to_armor - zn_to_wait)
                                                                - M_PI + max_out_angle)
            + M_PI - max_out_angle;
...
AimAndState state_vec_to_aim(const std::vector<State>& state_vec,
                             const double& max_orientation_angle,
                             const double& max_out_error,
    ...
    if (direct_vec.empty()) {
        ...
        return choose_indirect_aim(state_vec,
...
TopModel::TopModel(const param::ParamVisitor& param_visitor): param_visitor_(std::move(param_visitor)) {}
...
void TopModel::init(aimer::CoordConverter* const converter, aimer::EnemyState* const enemy_state) {
    ...
    const auto init_x = observation_to_init_state(std::get<0>(observation_and_r_and_id),
...
void TopModel::update(aimer::CoordConverter* const converter,
                      aimer::EnemyState* const enemy_state) {
    ...
    }
    {
        ...
        const auto get_q = [this](const std::string& name) {
            return param::get_param<double>("auto-aim.cj-top-model.q." + name,param_visitor_);
        };
    ...
    if (tracked_id != this->tracked_armor_id) {
        ...
        const Eigen::Vector3d tracked_xyz =
            aimer::math::ypd_to_xyz(aimer::math::YpdCoord(tracked_observation[0],
        ...
        if (most_like_index != 0) {
            ...
            if (armors_num == 4 && most_like_index % 2 == 1) {
                ...
                const double dz_abs_max =
                    ::param::get_param<double>("auto-aim.cj-top-model.fix.dz.abs-max",param_visitor_);
    ...
    this->ekf.update_forward(Measure(),
                             fixed_observation,
                             aimer::math::array_to_diag_mat(tracked_r));
    ...
    // [更新后修复.防止半径求解误差过大]
    {
        using ::param::get_param;
        auto fixed_state = this->ekf.get_x();
        fixed_state[8] = std::clamp(fixed_state[8],
                                    get_param<double>("auto-aim.cj-top-model.fix.radius.min",param_visitor_),
                                    get_param<double>("auto-aim.cj-top-model.fix.radius.max",param_visitor_));
        // pnp 观测的 z 存在巨大误差，紧凑跟随时可能会产生明显的 z 速度
        if (param::get_param<bool>("auto-aim.cj-top-model.fix.no-z-axis-motion",param_visitor_)) {
            fixed_state[5] = 0.0;
        }
    ...
    {
        if (enemy_state->get_enemy_type() == aimer::EnemyType::OUTPOST
            && param::get_param<bool>(
                "auto-aim.cj-top-model.fix.you-are-sure-that-the-outpost-will-not-move-in-a-continuous-observation",param_visitor_))
        {
            auto fixed_state = this->ekf.get_x();
            fixed_state[1] = 0.0;
            fixed_state[3] = 0.0;
...
bool TopModel::credit(aimer::CoordConverter* const converter) const {
    const double credit_dt = ::param::get_param<double>("auto-aim.cj-top-model.credit-dt",param_visitor_);
    return converter->get_img_t() - this->update_t <= credit_dt;
}

std::vector<PredictedArmor> TopModel::predict_armors(const double& t,
                                                     aimer::CoordConverter* const converter,
                                                     aimer::EnemyState* const enemy_state) const {
...
void TopModel::draw_armors(cv::Mat& img,
                           const double& t,
                           aimer::CoordConverter* const converter,
                           aimer::EnemyState* const enemy_state) const {
    const auto predicted_armor = this->predict_armors(t, converter, enemy_state);
    const cv::Scalar color = this->top_level == 0
        ? cv::Scalar { 127, 127, 127 }
        : this->top_level == 1 ? cv::Scalar { 151, 128, 255 } : cv::Scalar { 235, 206, 135 };
    for (const auto& armor: predicted_armor) {
        ...
        const std::vector<cv::Point2f> four_corners_points =
            aimer::top::radial_armor_pts(armor.pos,
                                         armor.type,
...
aimer::AimInfo TopModel::get_aim(aimer::CoordConverter* const converter,
                                 aimer::EnemyState* const enemy_state) const {
    using ::param::get_param;
        ...
        if (this->top_level == 0) {
            if (armors_num == 4) {
                return get_param<double>(
                    "auto-aim.cj-top-model.aim.top0.max-orientation-angle.armors-4",param_visitor_);
            }
            return get_param<double>(
                "auto-aim.cj-top-model.aim.top0.max-orientation-angle.armors-other",param_visitor_);
        }
        return get_param<double>(
            fmt::format("auto-aim.cj-top-model.aim.top{}.max-orientation-angle", this->top_level),param_visitor_);
    }());
    const double max_out_error = get_param<double>(
        fmt::format("auto-aim.cj-top-model.aim.top{}.max-out-error", this->top_level),param_visitor_);
    const bool allow_indirect = this->top_level > 0;
    const auto water_gun_hit_aim_and_state = state_vec_to_aim(water_gun_hit_state_vec,
                                                              max_orientation_angle,
                                                              max_out_error,
                                                              allow_indirect,
                                                              converter,
    ...
    const auto command_hit_aim_and_state = state_vec_to_aim(command_hit_state_vec,
                                                            max_orientation_angle,
                                                            max_out_error,
                                                            /*allow_indirect=*/true,
                                                            converter,
    ...
    const bool you_had_better_shoot_at_this_command = [&]() {
        ...
        const double max_tracking_error =
            ::param::get_param<double>("auto-aim.cj-top-model.aim.max-tracking-error",param_visitor_);
        ...
        if (this->top_level > 0) {
            const auto angle_to_rotate_time = [this](const double& angle) -> double {
                const double a =
                    param::get_param<double>("auto-aim.cj-top-model.aim.angle-to-rotate-time.a",this->param_visitor_);
                const double b =
                    param::get_param<double>("auto-aim.cj-top-model.aim.angle-to-rotate-time.b",this->param_visitor_);
                // 注意这里线性函数的参数是角度制哦
        ...
        if (converter->aim_error_exceeded(
                ...
                converter->target_pos_to_aim_ypd(
                    state_to_armor_pos(command_hit_aim_and_state.state)),
                enemy_state->get_sample_armor_ref(),
                max_out_error,
                state_to_zn_to_armor(command_hit_aim_and_state.state, converter),
                enemy_state->get_armor_pitch()))
...
std::tuple<Observation, std::array<double, N_Z>, int>
TopModel::get_observation_and_r_and_id(aimer::CoordConverter* const converter,
                                       aimer::EnemyState* const enemy_state) const {
    ...
    const int tracked_index = [&]() {
        ...
        const double keep_tracking_ratio =
            ::param::get_param<double>("auto-aim.cj-top-model.keep-tracking-area-ratio",param_visitor_);
    ...
    const auto get_r = [this](const std::string& name) {
        return ::param::get_param<double>("auto-aim.cj-top-model.r." + name,this->param_visitor_);
    };
    const auto orientation_yaw_and_its_r = [&]() -> std::tuple<double, double> {
        ...
        const auto z_to_armor_and_r = [&]() -> std::tuple<double, double> {
            if (armors.size() == 1) {
                const double z_to_armor_min =
                    std::max(M_PI - must_not_see_angle,
                             M_PI + must_see_angle - angle_between_armors);
                const double z_to_armor_max =
                    std::min(M_PI + must_not_see_angle,
                             M_PI - must_see_angle + angle_between_armors);
                // 当 3 装甲板时，这两个角是：朝左 / 朝右 75 度
                return { aimer::top::fit_single_z_to_v(armors[tracked_index],
                                                       z_to_armor_exp,
                                                       z_to_armor_min,
                                                       z_to_armor_max,
                                                       converter),
                ...
                // armors.size() == 2
                const double z_to_left =
                    aimer::top::fit_double_z_to_l(armors,
                                                  z_to_armor_exp,
        ...
        return { aimer::math::reduced_angle(converter->get_camera_z_i_yaw()
                                            + std::get<0>(z_to_armor_and_r)),
...
void TopModel::update_top_level() {
    ...
    const double next_level_activate_w = this->top_level == 2
        ? DBL_MAX
        : aimer::math::deg_to_rad(::param::get_param<double>(
            fmt::format("auto-aim.cj-top-model.aim.top{}.activate-w", this->top_level + 1),param_visitor_));
    const double deactivate_w = this->top_level == 0
        ? DBL_MIN
        : aimer::math::deg_to_rad(::param::get_param<double>(
            fmt::format("auto-aim.cj-top-model.aim.top{}.deactivate-w", this->top_level),param_visitor_));
