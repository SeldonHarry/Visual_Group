...
namespace aimer {
OutpostFixer::OutpostFixer(param::ParamVisitor param_visitor)
{
...
OutpostFixer::filter(const std::vector<ArmorData>& armors, const double& request_t) {
    const double z_that_distinguishes =
        param::get_param<double>("auto-aim.outpost-fixer.z-that-distinguishes-two-types",param_visitor_);
    const double active_lasting_time =
        param::get_param<double>("auto-aim.outpost-fixer.active-lasting-time",param_visitor_);
