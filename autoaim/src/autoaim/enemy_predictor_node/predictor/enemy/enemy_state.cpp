...
#include "enemy/balance.hpp"
#include "enemy/outpost_fixer.hpp"
...
EnemyState::EnemyState(const int& number,
                       const aimer::EnemyType& init_type,
                       const param::ParamVisitor& param_visitor):
    number(number),
    enemy_type(init_type),
    balance_judger(param_visitor),
    outpost_fixer(param_visitor),
...
void EnemyState::update(const std::vector<aimer::ArmorData>& raw_armors,
    ...
    if (this->number >= 3 && this->number <= 5) {
        ...
        // 若不开启手动设置型号，则进行自动识别
        if (!param::get_param<bool>("auto-aim.enemy-state.manually-set-balance-infantry",
                                    param_visitor_)) {
            ...
            aimer::EnemyType enemy_type_to_be = param::get_param<bool>(param_str, param_visitor_)
                ? aimer::EnemyType::BALANCE_INFANTRY
                : aimer::EnemyType::INFANTRY;
            this->switch_enemy_type(enemy_type_to_be);
        }
...
double EnemyState::get_aim_error() const { // 调用先远后近，明显不好
    return param::get_param<double>(this->get_info_ref().aim_error_param, param_visitor_);
}
...
EnemyState::screened_armors(const std::vector<aimer::ArmorData>& raw_armors,
    ...
    for (auto it = armors.begin(); it != armors.end();) {
        ...
        if (found_id) {
            ...
            if (it->info.area() < max_area * aimer::NEW_ARMOR_CRED_AREA_RATIO
                || it->info.pos.norm()
                    > param::get_param<double>("auto-aim.enemy-state.new-armor-max-valid-distance",
                                               param_visitor_))
            {
                it = armors.erase(it);
                continue;
...
// 根据指定 size 裁剪和排序装甲板
std::vector<aimer::ArmorData> EnemyState::sorted_armors(const std::vector<aimer::ArmorData>& src,
                                                        const int& max_size) {
