...
BalanceJudger::BalanceJudger(param::ParamVisitor param_visitor){
        ...
        if (credit_size >= 2) {
            this->double_conf += 0.5 * (t - this->last_t)
                / param::get_param<double>(
                                     "auto-aim.balance-judger.non-balance-confidence-ramp-time",param_visitor_);
        } else {
            this->double_conf -= 0.5 * (t - this->last_t)
                / param::get_param<double>("auto-aim.balance-judger.balance-confidence-ramp-time",param_visitor_);
        }
        // if (double_cons == 0)
        //   double_conf -= 0.00033;
        // else
