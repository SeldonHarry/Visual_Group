...
EnemyPredictor::EnemyPredictor(std::shared_ptr<::RobotStatus> robot_status,
                               const param::ParamVisitor& param_visitor,
                               std::shared_ptr<ros2addon::debug::Debugger> debugger):
    converter(robot_status, param_visitor),
    enemy_states {
        aimer::EnemyState(0, aimer::EnemyType::INFANTRY, param_visitor),
        aimer::EnemyState(1, aimer::EnemyType::HERO, param_visitor),
        aimer::EnemyState(2, aimer::EnemyType::ENGINEER, param_visitor),
        aimer::EnemyState(3, aimer::EnemyType::INFANTRY, param_visitor),
        // 运行时可能会更新 enemy_type
        aimer::EnemyState(4, aimer::EnemyType::INFANTRY, param_visitor),
        aimer::EnemyState(5, aimer::EnemyType::INFANTRY, param_visitor),
        aimer::EnemyState(6, aimer::EnemyType::OUTPOST, param_visitor),
        aimer::EnemyState(7, aimer::EnemyType::CRYSTAL_BIG, param_visitor),
        aimer::EnemyState(8, aimer::EnemyType::CRYSTAL_SMALL, param_visitor),
    },
    light { aimer::LightManager(&this->converter) },
    target_catcher { aimer::TargetCatcher(&this->converter, this->enemy_states) },
    aim_corrector { aimer::aim::AimCorrector(&this->converter, param_visitor, debugger) },
    stm32_shoot(param_visitor) {
    param_visitor_ = std::move(param_visitor);
    debugger_ = debugger;
    robot_status_ = robot_status;
    for (int i = aimer::MIN_ENEMY_NUMBER; i <= aimer::MAX_ENEMY_NUMBER; ++i) {
        this->enemy_models[i] = this->model_factory.create_model(&this->converter,
                                                                 &this->enemy_states[i],
                                                                 std::move(param_visitor_),
                                                                 debugger);
    }
}

::RobotCmd EnemyPredictor::predict(const aimer::DetectionResult& data) {
    debugger_->process_begin();
    debugger_->get_and_register("开始预测");

    debugger_->flask_aim.clear();
    debugger_->flask_map.clear();
    // 数据库更新
    ...
    debugger_->print_duration("更新模型后");

    this->update_aim_error();

    debugger_->print_duration("瞄准采样后");
    ...
    debugger_->print_duration("计算瞄准点和发送量后");

    this->processing_time_recorder.update(
        debugger_->get_process_time(),
        this->converter.get_img_t(),
        param::get_param<double>("auto-aim.debug.periodic-monitor.period", param_visitor_));
    debugger_->send_to_webpage(
        "预测器效率/近期最大运行时间",
        fmt::format(
            "于 {:.2f} 秒内: {:.3f} 毫秒",
            param::get_param<double>("auto-aim.debug.periodic-monitor.period", param_visitor_),
...
std::vector<aimer::ArmorData> EnemyPredictor::get_sorted_armors() {
    ...
    const cv::Point2f img_center = cv::Point2f(this->converter.get_img_ref().cols / 2.f,
    ...
    // );
    debugger_->send_to_webpage("预测器信息/EnemyState-捕获后装甲板排序",fmt::format("{}", debugger_->vec_to_str(sorted_numbers)));
...
void EnemyPredictor::update_aim_error() {
    if (param::get_param<bool>("auto-aim.ec-simulator.enabled", param_visitor_)) {
    ...
    if (param::get_param<bool>("auto-aim.aim-corrector.sample.enabled", param_visitor_)) {
...

    {
        ...
        debugger_->send_to_webpage(
            "预测器信息/瞄准校正数据",
            fmt::format("估计偏差: {:.2f} | {:.2f} 是否使用: {}",
                        aimer::math::rad_to_deg(aim_correction(0, 0)),
                        aimer::math::rad_to_deg(aim_correction(1, 0)),
                        param::get_param<bool>("auto-aim.aim-corrector.correction.enabled",
                                               param_visitor_)));
    }
}
...
void EnemyPredictor::update_models(const std::vector<aimer::ArmorData>& sorted_armors) {
    ...
    const size_t max_num_updated = static_cast<size_t>(
        param::get_param<int64_t>("auto-aim.enemy-model.max-num-updated", param_visitor_));
...
::RobotCmd EnemyPredictor::get_cmd_by_aim(const aimer::AimInfo& aim) {
    ...
    const double additional_prediction_time =
        param::get_param<double>("auto-aim.cmd.additional-predict-time", param_visitor_);
    ...
    if (param::get_param<bool>("auto-aim.cmd.send-yaw-v", param_visitor_)) {
        if (param::get_param<bool>("auto-aim.cmd.yaw-v-is-radian", param_visitor_)) { // 弧度制
    ...
    // 参数表可强制修改发送给电控的命令
    // if (param::get_param<int64_t>("CMD_DEBUGGER_ON",param_visitor_) == 1) {
    //     this->cmd_debugger.fix(cmd);
    // }

    {
        debugger_->send_to_webpage("预测器信息/发送给电控_yaw",
                                   fmt::format("{:.2f}", float(cmd.yaw)));
        debugger_->send_to_webpage("预测器信息/发送给电控_pitch",
                                   fmt::format("{:.2f}", float(cmd.pitch)));
        debugger_->send_to_webpage("预测器信息/发送给电控_shoot",
        ...
        debugger_->send_to_webpage("预测器信息/最近发弹指令时间",
...
aimer::AimInfo EnemyPredictor::get_aim() {
    ...
    debugger_->send_to_webpage("预测器信息/瞄准敌人编号", fmt::format("{}", target));
    if (target == -1) {
        return aimer::AimInfo::idle();
    }

    const Eigen::Vector2d aim_correction =
        param::get_param<bool>("auto-aim.aim-corrector.correction.enabled", param_visitor_)
    ...
    this->aim_corrector.add_aim(
        aimer::aim::IdTLatencyAimCorrection { this->aim_id_cnt,
                                              this->converter.get_img_t(),
    ...
    debugger_->send_to_webpage("预测器信息/我希望还需转动_pitch",
...
void EnemyPredictor::update_database(const aimer::DetectionResult& data) {
    ...
    debugger_->send_to_webpage(
        "机器人状态/电控_敌人颜色",
        this->converter.get_robot_status_ref().enemy_color == ::EnemyColor::BLUE ? "BLUE0"
                                                                                 : "RED1");
    ...
    // );
    debugger_->send_to_webpage("机器人状态/电控_yaw_轴补偿",
    ...
    debugger_->send_to_webpage("机器人状态/电控_pitch_轴补偿",
                               fmt::format("原始: {:.2f} 视觉认为他使用: {:.2f}",
                                           this->converter.get_robot_status_ref().pitch_compensate,
                                           this->converter.get_pitch_compensate() / M_PI * 180.));
    ...
    const auto ignore_enemy = [&]() {
        ...
        const auto list =
            param::get_param<std::vector<int64_t>>("auto-aim.ignore-enemy.list", param_visitor_);
    ...
    for (const auto& d: data.armors) {
        ...
        aimer::ArmorInfo armor_info =
            aimer::detected_to_info(d,
    ...
    // );
    debugger_->send_to_webpage("预测器信息/装甲板数-忽略非法和指定后", fmt::format("{}", armor_info_vec.size()));
    ...
    // );
    debugger_->send_to_webpage("预测器信息/装甲板数量-亮装甲追踪后", fmt::format("{}", light_armors.size()));
...
cv::Mat EnemyPredictor::draw_aim(const cv::Mat& img, const aimer::DetectionResult& data) {
    ...
    // 输出平衡机器人列表
    {
        ...
        debugger_->send_to_webpage("机器人状态/哪些敌人被认为平衡", balance_str);
    }
    ...
    // 绘制模拟发射的子弹
    {
        ...
        for (auto& bullet: bullets) {
            debugger_->flask_aim << ros2addon::debug::FlaskPoint(bullet.circle.center,
                                                                { 0, 0, 255 },
                                                                bullet.circle.r,
                                                                2);
            debugger_->flask_aim << ros2addon::debug::FlaskText(
    ...
    // 绘制自定义平面
    if (param::get_param<bool>("auto-aim.debug.flask.plane.show",param_visitor_)) {
        ...
        for (int i = 0; i < 4; ++i) {
            Eigen::Vector3d center = { 0.,
                                       0.,
                                       param::get_param<double>("auto-aim.debug.flask.plane.z",param_visitor_)};
            for (int j = 1; j <= 8; ++j) {
                center += directions[i];
                ...
                debugger_->flask_map
                    << debugger_->poses_to_map_lines(pis,
                                                        colors[i],
                                                        true,
                                                        ros2addon::debug::FLASK_MAP_THICKNESS);
                if (all_z_plus) {
                    debugger_->flask_aim
                        << debugger_->pts_to_map_lines(pts, colors[i], true, 3);
                }
            }
        }
    }
    ...
    // 绘制自定义立方体
    if (param::get_param<bool>("auto-aim.debug.flask.cube.show",param_visitor_)) {
        double dis { param::get_param<double>("auto-aim.debug.flask.cube.dis",param_visitor_) };
        ...
        for (int i = 0; i < 4; ++i) {
            Eigen::Vector3d center { 0.,
                                     0.,
                                     param::get_param<double>("auto-aim.debug.flask.cube.z",param_visitor_) };
            center += directions[i];
            for (int k = 0; k < 8; k += 2) {
                std::vector<Eigen::Vector3d> pis;
                std::vector<cv::Point2f> pts;
    ...
    // 显示铅垂线
    if (param::get_param<bool>("auto-aim.debug.flask.plumbing",param_visitor_)) {
        ...
        debugger_->flask_aim << debugger_->pts_to_map_lines(pts, { 255, 127, 63 }, false, 3);
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(center_pt, { 255, 127, 63 }, 9, 3);
    }
    ...
    for (int i = aimer::MIN_ENEMY_NUMBER; i <= aimer::MAX_ENEMY_NUMBER; ++i) {
        for (const auto& d: this->enemy_states[i].get_armor_data_ref()) {
            ...
            std::vector<std::string> txt = {
                ...
                fmt::format("col ori|fix: {}|{} S: {:.1f}k",
                            d.info.detected.color,
                            d.color,
                            d.info.area() / 1000.0f),
                fmt::format("ypd {:.1f}|{:.1f}|{:.3f}",
                            aimer::math::rad_to_deg(ypd.yaw),
                            aimer::math::rad_to_deg(ypd.pitch),
                            ypd.dis),
                fmt::format("xyz {:.2f}|{:.2f}|{:.2f}",
                            d.info.pos(0, 0),
                            d.info.pos(1, 0),
                            d.info.pos(2, 0)),
                fmt::format("yp_c {:.1f}|{:.1f}",
    ...
    if (this->debug_cmd.shoot != ::ShootMode::IDLE) { // not idle
        ...
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(exp_pu, { 0, 105, 255 }, 10, 4);
        ...
        debugger_->draw_arrow(aim_mat, exp_pu, arrow_end, { 0, 0, 255 }, 2);
        cv::Point2f cmd_pu = this->converter.aim_ypd_to_pu(
            aimer::math::YpdCoord(-this->debug_cmd.yaw / 180. * M_PI,
                                  -this->debug_cmd.pitch / 180. * M_PI,
                                  this->debug_cmd.dist));
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(cmd_pu, { 0, 255, 255 }, 10, 4);
        // 显示适合打击
    ...
    debugger_->flask_aim >> aim_mat;
    return aim_mat;
}

cv::Mat EnemyPredictor::draw_map() {
    cv::Mat map_mat =
        cv::Mat::zeros(cv::Size(ros2addon::debug::FLASK_MAP_WIDTH, ros2addon::debug::FLASK_MAP_HEIGHT),
                       CV_8UC3);
    int width_meter_cnt =
        int(ros2addon::debug::FLASK_MAP_WIDTH / 2.
            / param::get_param<double>("auto-aim.debug.flask.map.pixel-per-meter",param_visitor_));
    int height_meter_cnt =
        int(ros2addon::debug::FLASK_MAP_HEIGHT / 2.
            / param::get_param<double>("auto-aim.debug.flask.map.pixel-per-meter",param_visitor_));
    {
        for (int i = -width_meter_cnt; i <= width_meter_cnt; ++i) {
            for (int j = -height_meter_cnt; j <= height_meter_cnt; ++j) {
                debugger_->flask_map
                    << debugger_->pos_to_map_point({ double(i), double(j), 0. },
                                                      { 255., 255., 255. },
                                                      2,
                                                      1);
            }
    ...
    debugger_->flask_map << debugger_->pos_to_map_point({ 0., 0., 0. },
                                                              { 255., 255., 255. },
                                                              ros2addon::debug::FLASK_MAP_PT_RADIUS,
                                                              ros2addon::debug::FLASK_MAP_THICKNESS);
    debugger_->flask_map << debugger_->poses_to_map_lines(
        {
            { 0., 0., 0. },
    ...
    for (int i = aimer::MIN_ENEMY_NUMBER; i <= aimer::MAX_ENEMY_NUMBER; ++i) {
        const std::vector<aimer::ArmorData>& armors = this->enemy_states[i].get_armor_data_ref();
        for (const auto& d: armors) {
            debugger_->flask_map << debugger_->pos_to_map_point(
                d.info.pos,
