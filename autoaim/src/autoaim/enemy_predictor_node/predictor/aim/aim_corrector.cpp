...
#include "aim/detect_bullet.hpp"
#include "aim/do_reproj.hpp"
...
auto ProjectileSimulator::get_circle_by_t(const double& t) const -> aim::HitCircle {
    ...
    Eigen::Vector3d edge_xyz_c = xyz_c
        + crossed * param::get_param<double>("launching-mechanism.bullet.radius", param_visitor_);
...
AimCorrector::AimCorrector(aimer::CoordConverter* const converter,
    ...
    converter { converter },
    bullet_detector { aim::DoReproj(this->converter->get_f_cv_mat_ref(),
    ...
    while (true) {
        ...
        if (this->bullets.size() + 1u <= aim::AIM_CORRECTOR_BULLETS_MAX_SZ) {
            ...
            this->bullets.push_back(aim::IdProj {
                origin_aim.id,
                aim::ProjectileSimulator(this->converter,
...
auto AimCorrector::sample_aim_errors() -> void {
    ...
    this->bullet_detector.process_new_frame(this->converter->get_img_ref(),
    ...
    for (const auto& d: detected) {
        ...
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(d.center,
                                                             { 255, 0, 255 },
                                                             d.radius,
                                                             3);
        debugger_->flask_aim << ros2addon::debug::FlaskPoint(u.center, { 0, 255, 255 }, u.r, 3);
    }
    for (const auto& bullet: this->bullets) {
        ...
        if (best_bullet != undistorted_detected.end()) {
            ...
            debugger_->flask_aim << ros2addon::debug::FlaskPoint(fit.center,
                                                                 { 255, 255, 255 },
                                                                 fit.r,
                                                                 3);
                                         ...
                                         caught_yp.pitch - fit_yp.pitch };
            {
                std::vector<double> r_vec = {
                    param::get_param<double>("auto-aim.aim-corrector.error.r", param_visitor_)
                };
    ...
    }
    {
        ...
        for (auto& d: this->error_angles) {
            i += 1;
            debugger_->send_to_webpage("自瞄信息/aim_corrector_采样数据",
                                       fmt::format("{:.2f}, {:.2f}",
...
auto AimCorrector::get_aim_error() const -> Eigen::Vector2d {
    return Eigen::Vector2d(this->error_filters[0].predict(0.)(0, 0),
