...
private:
    ...
    double get_param_k() const {
        return param::get_param<double>("launching-mechanism.bullet.resistance-k", param_visitor_);
    }

public:
    ResistanceFuncLinear(const double& w,
                         const double& h,
                         const double& v0,
...
CoordConverter::CoordConverter(std::shared_ptr<RobotStatus> robot_status,
                               param::ParamVisitor param_visitor):
    robot_status_obj { robot_status } {
    param_visitor_ = param_visitor;
    cv::FileStorage fin("assets/param.yml", cv::FileStorage::READ);
...
double CoordConverter::get_yaw_compensate() const {
    return param::get_param<bool>("ec.using-angle-compensate", param_visitor_)
...
double CoordConverter::get_img_to_predict_latency() const {
    ...
    return aimer::math::clamp_default(latency,
...
double CoordConverter::get_send_to_control_latency() const {
    return param::get_param<double>("auto-aim.latency.send-to-control", param_visitor_);
}
...
double CoordConverter::get_control_to_fire_latency() const {
    ...
    return param::get_param<double>("auto-aim.latency.control-to-fire", param_visitor_);
}
...
double
CoordConverter::get_img_to_prediction_latency(const Eigen::Vector3d& aim_xyz_i_barrel) const {
...
Eigen::Vector2d CoordConverter::get_camera_z_i_pitch_vec() const {
    ...
    Eigen::Vector2d camera_z_i_pitch_vec { std::sqrt(aimer::math::sq(camera_z_i(0, 0))
                                                     + aimer::math::sq(camera_z_i(1, 0))),
                                           camera_z_i(2, 0) };
...
void CoordConverter::catch_predict_timestamp() {
    this->predict_timestamp_binder.update(std::chrono::duration_cast<std::chrono::milliseconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                                  .count()
                                              / 1e3,
                                          this->get_frame()); // by agx
}
...
bool CoordConverter::aim_error_exceeded(const aimer::math::YpdCoord& ypd,
...
bool CoordConverter::aim_error_exceeded(const aimer::math::YpdCoord& real_ypd,
                                        const aimer::math::YpdCoord& ideal_ypd,
                                        const aimer::SampleArmor& sample_armor,
                                        const double& error_rate,
                                        const double& yaw_inclined_camera,
                                        // 通常我们方便测的相对 yaw 和绝对 pitch
                                        const double& pitch_inclined_world) const {
...
double CoordConverter::aim_swing_cost(const aimer::math::YpdCoord& ypd) const {
    return aimer::math::get_norm(ypd.yaw - this->get_yaw_compensate(), // aim = -send
...
// 此为利用相机的坐标系系统所求出的近似值
aimer::math::YpdCoord CoordConverter::get_camera_ypd_v(const Eigen::Vector3d& xyz_i,
...
CoordConverter::pts_to_pis_norm(const std::vector<cv::Point2f>& pts) const {
    std::vector<cv::Point2f> pus;
...
Eigen::Vector3d
CoordConverter::xyz_i_camera_to_xyz_i_barrel(const Eigen::Vector3d& xyz_i_camera) const {
    Eigen::Vector3d xyz_c_camera = this->pi_to_pc(xyz_i_camera);
    // 注意是目标坐标，平移方向与相机平移到枪口相反
    Eigen::Vector3d xyz_c_barrel = {
        xyz_c_camera(0, 0)
            - param::get_param<double>("launching-mechanism.camera-to-barrel-x", param_visitor_),
        xyz_c_camera(1, 0)
            - param::get_param<double>("launching-mechanism.camera-to-barrel-y", param_visitor_),
        xyz_c_camera(2, 0)
    };
    // 以枪口为原点的世界坐标系
...
Eigen::Vector3d
CoordConverter::xyz_i_barrel_to_xyz_i_camera(const Eigen::Vector3d& xyz_i_barrel) const {
...
aimer::ShootParam
CoordConverter::target_pos_to_shoot_param(const Eigen::Vector3d& target_pos) const {
    ...
    const double target_xy = std::sqrt(target_xyz_i_barrel(0, 0) * target_xyz_i_barrel(0, 0)
                                       + target_xyz_i_barrel(1, 0) * target_xyz_i_barrel(1, 0));
    const double target_z = target_xyz_i_barrel(2, 0);
    const double shoot_angle = [this, &target_xy, &target_z, &bs]() {
        double shoot_angle = 30. / 180. * M_PI;
        ceres::Problem problem;
        problem.AddResidualBlock(
            new ceres::AutoDiffCostFunction<ResistanceFuncLinear, 1, 1>(
                new ResistanceFuncLinear(target_xy, target_z, bs, this->param_visitor_)),
            nullptr,
            &shoot_angle);
...
aimer::math::YpdCoord CoordConverter::aim_xyz_i_to_aim_ypd(const Eigen::Vector3d& aim_xyz_i) const {
    ...
    aimer::math::YpdCoord aim_ypd_b {
        aim_ypd_c_barrel.yaw
            - param::get_param<double>("auto-aim.aim-offset.yaw", param_visitor_) / 180. * M_PI,
        aim_ypd_c_barrel.pitch
            - param::get_param<double>("auto-aim.aim-offset.pitch", param_visitor_) / 180. * M_PI,
...
double CoordConverter::filter_to_prediction_time(const PositionPredictorInterface& filter) const {
    int iterations_num =
        param::get_param<int64_t>("auto-aim.predict.num-iterations", param_visitor_);
...
aimer::ShootParam
CoordConverter::filter_to_shoot_param(const PositionPredictorInterface& filter) const {
    return this->target_pos_to_shoot_param(
        filter.predict_pos(this->filter_to_prediction_time(filter)));
}
...
// aim 总是关于枪口的，这里存在误差
aimer::math::YpdCoord
CoordConverter::filter_to_aim_ypd_v(const PositionPredictorInterface& filter) const {
...
aimer::math::YpdCoord
CoordConverter::filter_to_hit_aim_ypd(const PositionPredictorInterface& filter) const {
...
void CoordConverter::update(const cv::Mat& img,
